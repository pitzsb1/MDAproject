...
- **연산 효율성과 실시간성 (Edge Computing 최적화)**

참고 프로젝트의 방식은 EEG 신호를 스펙트로그램 이미지로 굽는 과정 자체가 무거움. 반면 통계 피처 추출은 가벼운 산술 연산뿐이므로 휴대용 EEG 기기나 스마트폰(재택 모니터링)에서 즉각적인 분석이 가능.

- **도메인 지식의 직접 반영**

딥러닝은 모델이 스스로 특징을 찾기를 기다려야 하지만 이 프로젝트에서는 "발작 시에는 진폭이 커진다(Standard Deviation ↑)"나 "특정 주파수 대역의 에너지가 집중된다" 같은 의학적 가설을 피처에 직접 녹여낼 수 있음.

### 기존 프로젝트의 성능 개선 (기술적 차이)

- **추론 속도 및 자원 효율성 (Latency & Resource Efficiency)**

딥러닝 방식 대비 추론 속도가 약 100배 이상 향상됨. 기존 프로젝트에서의 방식은 Raw EEG 데이터를 이미지(Spectrogram)로 변환하는 전처리 과정과 수천만 개의 파라미터를 가진 CNN 모델(EfficientNet 등)을 통과하는 과정에서 높은 연산량을 요구. 반면 통계 피처 기반 ML 모델은 산술적인 요약 수치만 계산하므로 CPU 환경에서도 밀리초(ms) 단위로 결과를 내놓을 수 있음.

- **모델의 설명 가능성 및 진단 근거 제시 (Explainability / XAI)**

판단의 근거가 되는 ‘피처 중요도(Feature Importance)’의 명확성이 비약적으로 향상됨. 딥러닝은 모델 내부에서 어떤 특징을 보고 발작(Seizure)이라 판단했는지 알기 어려운 '블랙박스' 구조. 본 프로젝트에서는 "20개 전극 중 T3, T4(측두엽)의 표준편차(Std)와 왜도(Skewness)가 임계치를 넘었기 때문"과 같이 의학적으로 해석 가능한 수치를 직접 제공.
