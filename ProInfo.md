...
진단의 복잡성: 뇌파(EEG) 데이터는 초당 수백 번 진동하는 다채널 신호로, 전문가가 아니면 이상 징후를 직관적으로 파악하기 어려움.

데이터 효율성: 딥러닝(CNN)을 위한 이미지 변환 과정은 높은 연산 비용과 복잡한 수학적 전처리가 필요하여 초기 접근 장벽이 높음.

머신러닝의 강점: 뇌파의 진폭, 변동성 등 핵심 통계치만으로도 발작(Seizure) 등의 주요 패턴을 빠르게 감지할 수 있는 가벼운 시스템이 필요함.
